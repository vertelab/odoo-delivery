...
class fj_query(models.TransientModel):
    ...
    def fraktjakt_query(self):
        ...
        _logger.warn(etree.tostring(shipment, pretty_print=True, encoding='UTF-8'))
...
# class fj_query_lineBAADD(models.TransientModel):
#     _name = 'fj_query.line.bad'

#     wizard_id = fields.Many2one(comodel_name='fj_query')
#     carrier_id = fields.Many2one(comodel_name='delivery.carrier')
#     #partner_id = fields.Many2one(comodel_name='res.partner')   #mic
...
#         _logger.warning("choose_product"*100)
...
#         _logger.warn(etree.tostring(order, pretty_print=True, encoding='UTF-8'))
...
#         record = etree.XML(response.content)
#         _logger.warn(etree.tostring(record, pretty_print=True, encoding='UTF-8'))
#         _logger.warn("Marcus" *100)
#         _logger.warn(record.tag)
#         self.wizard_id.message = response.content
#         if len(record) and record.tag == 'shipment':
#             code = record.find('code').text
#             warning = record.find('warning_message').text
#             error = record.find('error_message').text
