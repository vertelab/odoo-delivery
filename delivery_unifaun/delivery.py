...
class delivery_carrier(models.Model):
    ...
    # with Function on <odoo12
    #available = fields.function(get_price, string='Available',type='boolean', multi='available', help="Is the carrier method possible with the current order.")
    #price = fields.function(get_price, string='Price', multi='price'),
    
    # with compute on odoo12 (not done)
    available = fields.One2many(comodel_name='', compute='get_price', string="Available")
    price = fields.One2many(comodel_name='', compute='get_price', string='Price')
...
class delivery_grid_line(models.Model):
    ...
    type = fields.Selection([('weight','Weight'),('volume','Volume'), ('wv','Weight * Volume'), ('price','Price'), ('quantity','Quantity')], 'Variable', required=True)
