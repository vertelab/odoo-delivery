...
class UnifaunPackage(models.Model):
    ...
    weight = fields.Float(string='Weight', compute='_calculate_weight', store=True)
    weight_calc = fields.Float(string='Weight Calc', compute='_calculate_weight', store=True)
    weight_spec = fields.Float(string='Specified Weight', help="Use this field to override the calculated weight.", states=READ_ONLY_STATES)
...
class stock_picking(models.Model):
    ...
    def _calculate_weight(self):
        for picking in self:
            ...
            for op in picking.move_lines.move_line_ids.filtered(lambda o: not o.result_package_id):
                qty = op.product_uom_id._compute_quantity(op.product_qty, op.product_id.uom_id)
                # qty = op.product_uom_id._compute_quantity(op.product_uom_id, op.product_qty, op.product_id.uom_id)
                _logger.warning(f"victor 1{type(op.product_id.weight)}")
                _logger.warning(f"victor 2{type(qty)}")
                total_weight += op.product_id.weight * qty
            _logger.warning(f"victor 3{type(total_weight)}")
            _logger.warning(f"victor 4{total_weight}")
